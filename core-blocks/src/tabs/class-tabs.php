...
class Core_Block_Tabs {
	...
	public function init( $loader = null ): void {
		if ( null !== $loader ) {
			$loader->add_action( 'init', $this, 'register_block_core_tabs' );
			$loader->add_filter( 'render_block_context', $this, 'block_core_tabs_provide_context', 10, 2 );
		}
	}
	 ...
	 */
	public function block_core_tabs_generate_tabs_list( array $innerblocks = array() ): array {
		$tabs_list = array();
	 ...
	 */
	public function block_core_tabs_provide_context( array $context, array $parsed_block ): array {
		if ( 'core/tabs' === $parsed_block['blockName'] ) {
			$tabs_list                 = $this->block_core_tabs_generate_tabs_list( $parsed_block['innerBlocks'] ?? array() );
	 ...
	 */
	public function block_core_tabs_render_block_callback( array $attributes, string $content, \WP_Block $block ): string {
	 ...
	 * @since 6.8.0
	 */
	public function register_block_core_tabs() {
		$block = register_block_type_from_metadata(
			PRC_BLOCK_LIBRARY_DIR . '/core-blocks/build/tabs',
			array(
				'render_callback' => array( $this, 'block_core_tabs_render_block_callback' ),
			)
		);
	}
}
