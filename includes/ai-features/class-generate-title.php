<?php
/**
 * Generate Title Ability.
 *
 * Generates a short, factual headline/title for a Story Item block based on the linked post.
 *
...
class Generate_Title {
	 ...
	 * @var string
	 */
	public static $ability_name = 'prc-ai/generate-title';
	...
	public function register_ability() {
		wp_register_ability(
			...
			array(
				...
				'meta'                => array(
					'annotations'    => array(
						'instructions' => 'This ability generates a short, factual headline/title for a Story Item block from the linked post title and content.',
						'readonly'     => true,
						'destructive'  => false,
						'idempotent'   => false,
					),
...
- Write in headline case (capitalize major words).';
	...
	public function execute( $input ) {
		...
		if ( '' === $raw_source && '' === $title ) {
			return array(
				'error' => __( 'No post content available to generate a title.', 'prc-block-library' ),
				'title' => '',
			);
		}
		...
		$generated_title = trim( (string) $generated_title );
