...
class Pagination {
	...
	public function get_current_item() {
		$active_item = array_filter($this->items, function($item) {
			return $item['is_active'] ?? false;
		});
	...
	public function get_next_item() {
		$next_item = array_filter($this->items, function($item) {
	...
	public function get_previous_item() {
		$previous_item = array_filter($this->items, function($item) {
			return $item['page_num'] === $this->current_page_num - 1;
		});
	...
	public function get_adjacent_post_button($link = false, $label = 'Next Page &rarr;', $button_classnames = '', $next_or_previous = 'next') {
		...
		$classnames = \PRC\Platform\Block_Utils\classNames($button_classnames, array(
	...
	protected function get_item_by_pagenum($pagenum) {
		...
		$filtered = array_filter($this->items, function($item) use ($pagenum) {
	...
	protected function render_item($pagenum, $classnames = '') {
		...
		$matched_item = array_filter($this->items, function($item) use ($pagenum) {
			return $item['page_num'] === $pagenum;
		});
		...
		$classnames = \PRC\Platform\Block_Utils\classNames($classnames, 'common-block-style__pagination__page-numbers', [
			'is-active' => $is_active,
		]);
