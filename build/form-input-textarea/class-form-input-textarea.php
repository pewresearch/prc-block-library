...
/**
 * Block Name:        Input Textarea Field
 * Version:           0.1.0
...
class Form_Input_Textarea {
	 ...
	 *
	 * @hook render_block_prc-block/form-input-textarea
	 *
	...
	public function render_block_callback( $attributes, $content, $block ) {
		...
		$tag->next_tag(
			array(
				'class_name' => 'wp-block-prc-block-form-input-textarea',
			)
		);
		$block_id = $tag->get_attribute( 'id' );
		if ( ! $block_id ) {
			$block_id = wp_unique_id( 'prc-block-form-input-textarea-' );
		}
	...
	public function block_init() {
		register_block_type_from_metadata(
			PRC_BLOCK_LIBRARY_DIR . '/build/form-input-textarea',
			array(
