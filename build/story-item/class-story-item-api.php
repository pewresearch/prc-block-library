...
class Story_Item_API {
	...
	/**
	 * Constructor for Story Item block data.
	 *
	...
	public function __construct( $attributes = array(), $content = null, $context = array() ) {
		...
		// Setup variables:
		$this->is_mobile = 'mobile' === \PRC\Primitives\BlockUtils\get_current_device();
	...
	public function get_title_markup() {
		...
		$header_class = \PRC\Primitives\BlockUtils\classNames(
			'header',
			array(
				'large'  => 1 === $header_size,
	...
	private function get_first_term_name( $taxonomy ) {
		$first_term = false;
		$terms      = wp_get_object_terms( $this->post_id, $taxonomy, array() );
		$tmp        = $terms;
		...
		if ( ! is_wp_error( $terms ) && ! empty( $terms ) ) {
			...
			if ( count( $terms ) > 1 ) {
				$primary_term_id = \PRC\Primitives\BlockUtils\get_primary_term_id( $this->post_id, $taxonomy );
