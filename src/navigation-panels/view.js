...
const { actions, state } = store('prc-block/navigation-panels', {
	state: {
		...
		get isSelected() {
			...
			return (
				state[navigationPieces[0]].activeIndex ===
					+navigationPieces[1] ?? false
			);
		},
		...
		get localReturnLabel() {
			...
			if (!returnLabel) {
				const { option } = getContext();
				const optionReturnLabel = option.returnLabel;
				if (optionReturnLabel) {
					return optionReturnLabel;
				}
				return false;
			}
			return returnLabel;
	...
	actions: {
		setNavigation: () => {
			...
			if (navigation?.level === 'child') {
				if (
					Array.isArray(navigation.parentNavigationList) &&
					navigation.parentNavigationList.length > 0
				) {
					actions.addSubNavigation(
						navigationPieces.join('___'),
						navigation.parentNavigationList
					);
				}
			}
		},
		...
		callSetNavigation: async (
			navPanelName,
			panel,
			reset = false,
			returnLabel = 'Back'
		) => {
			const id = state?.byName?.[navPanelName] ?? false;
			if (!id || !state[id]) {
				return false;
			}
			const navigation = state[id];
			const index =
				reset || isNaN(panel) ? navigation.defaultIndex : +panel;
			...
			if (reset || index < 0) {
				...
				if (navigation?.level === 'child') {
					if (
						...
						actions.addSubNavigation([id, 0].join('___'), []);
					}
				}
			} else {
				navigation.returnLabel = returnLabel;
				navigation.panelOpen = true;
				if (navigation?.level === 'child') {
					if (
						Array.isArray(navigation.parentNavigationList) &&
		...
		addSubNavigation: (inputId, subNavigationList) => {
			...
			const parentNavigation = element.closest(
				'.is-parent-nav.wp-block-prc-block-navigation-panels'
			);
			if (
				parentNavigation &&
				parentNavigation.id &&
				state[parentNavigation.id]
			) {
