...
domReady(()=>{
    const azIndex = document.querySelector('.wp-block-prc-block-topic-index-az-controller.ui.accordion');
    if ( azIndex ){
        ...
        if ( existingArg ) {
            setTimeout(()=>{
                document.querySelector(`.content[data-slug="${existingArg}"]`).scrollIntoView();
            }, 200);
        }
    }
});