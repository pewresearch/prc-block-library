 ...
 */
import { Slider, Select, Autocomplete } from '@prc/components';
	...
	return (
		<div>
			<Select
				id={'select-hook-demo'}
				multiple={false}
				placeholder={'Some fun movies'}
				options={top100Films.map((film) => ({
					value: film.label,
					label: film.label,
				}))}
				disabled={false}
				onChange={(e, val) => console.log(`you clicked on ${val}`)}
			/>
			<Autocomplete
				id={'autocomplete-hook-demo'}
				placeholder={'Some cool movies'}
				defaultValue={null}
				multiple={false}
				options={top100Films}
