...
class Core_Cover {
	...
	public function render( $block_content, $block ) {
		...
		if ( ! function_exists( '\PRC\Primitives\BlockUtils\get_current_device' ) ) {
			return $block_content;
		}
		...
		$image_attrs = null;
		if ( 'mobile' === \PRC\Primitives\BlockUtils\get_current_device() && preg_match( '/<img(.*?)>/', $block_content, $matches ) && false !== $mobile_image ) {
		...
		if ( 'tablet' === \PRC\Primitives\BlockUtils\get_current_device() && preg_match( '/<img(.*?)>/', $block_content, $matches ) && false !== $tablet_image ) {
			$image_attrs   = $matches[1];
