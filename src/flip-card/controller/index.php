...
class Flip_Card_Controller extends PRC_Block_Library {
	...
	public function render_block_callback( $attributes, $content, $block ) {
		$block_attrs = get_block_wrapper_attributes(
			array(
				'style' => sprintf( 'max-width: %spx; min-height: %spx; border-color: %s; background-color: %s', $attributes['maxWidth'], $attributes['minHeight'], $attributes['borderColor'], $attributes['backgroundColor'] ),
			)
		);
		ob_start();
		?>
