...
- **Grid-aware dividers** — setting an **Interior Divider** color on a grid group, plus optional **Divider Style** (solid/dashed/dotted/none) and **Divider Inset** in the Border panel, draws dividers between grid children. Placement is computed in the editor ([`utils/divider-placement.js`](utils/divider-placement.js)) from native spans + column count: side-by-side children get a vertical divider; stacked / full-width children get a horizontal divider; the first item in each row gets none. When Gutenberg auto-fill wraps a `minimumColumnWidth` grid to one column, the leftover vertical gutter rule becomes a horizontal rule across the stacked child. The frontend measures that wrap with an Interactivity API store (`prc-block/core-group`) and a `ResizeObserver` on the group's computed tracks — container width, not the viewport. Flags persist per child under `style.prcGridDivider` and render server-side without any parent lookup. In the editor, divider visibility follows the device preview toolbar (`.is-*-preview` rules in `style.scss`); on the frontend, `@media` breakpoints plus `.is-stacked` apply.
