...
function ChartControls({ attributes, setAttributes, clientId }) {
	...
	const createCanvas = () => {
		...
		html2canvas(blockEl).then((canvas) => {
			canvas.toBlob(
				(blob) => {
					upload(blob, `chart-${clientId}-${Date.now()}.png`, 'image/png');
				},
				'image/png',
				1,
			);
		...
		<InspectorControls>
			<PanelBody title={__('Chart Configuration')}>
				...
				<FormTokenField
					...
					value={customColors || []}
					placeholder="#000000"
					onChange={(colors) => setAttributes({ customColors: colors })}
				/>
			...
			<PanelBody title={__('Chart Tabs')} initialOpen={false}>
				<ToggleControl
					...
					onChange={() => setAttributes({ tabsActive: !tabsActive })}
				/>
			</PanelBody>
			<TextFieldControls
			...
			/>
			<XAxisControls attributes={attributes} setAttributes={setAttributes} />
			<YAxisControls attributes={attributes} setAttributes={setAttributes} />
			<DataControls attributes={attributes} setAttributes={setAttributes} />
			<LabelControls
				attributes={attributes}
				setAttributes={setAttributes}
				chartType={chartType}
			/>
			<LegendControls attributes={attributes} setAttributes={setAttributes} />
			<TooltipControls attributes={attributes} setAttributes={setAttributes} />
			{('bar' === chartType || 'stacked-bar' === chartType) && (
				<BarControls attributes={attributes} setAttributes={setAttributes} />
			)}
			...
			<PanelBody title="Image Exports" initialOpen={false}>
				...
				<PanelRow>
					<Button isSecondary isBusy={imageLoading} onClick={createCanvas}>
						Upload Chart PNG to Media Library
					</Button>
				</PanelRow>
				<PanelRow>
					{imageLoading && (
						<p>Creating image. This will take several moments ...</p>
					)}
				...
				{0 < pngUrl.length && (
					<>
						...
						<PanelRow>
							<ExternalLink href={pngUrl}>Preview Image</ExternalLink>
						</PanelRow>
					</>
				)}
			</PanelBody>
