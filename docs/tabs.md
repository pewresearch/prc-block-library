...
> **Deprecated.** This block has been superseded by `core/tabs`. It remains registered for backward compatibility with existing content. Source files live in `deprecated/src/tabs/`.

Container block that displays child Tab blocks in a tabbed interface. Supports horizontal and vertical orientations, three visual styles, full keyboard navigation, URL hash activation, and per-instance color theming.
