...
| Example     | Yes (show/hide labels, responsive `heights`, inner `core/paragraph` — inserter preview)                                                                                                                 |
...
```html
<div
	class="wp-block-prc-block-show-more"
	id="abc123"
...
>
	...
	<button
		class="prc-show-more__expand-button"
		type="button"
		data-wp-on--click="actions.toggleExpanded"
	>
		<span data-wp-bind--hidden="context.isExpanded"
			><!-- plus icon --></span
		>
		<span data-wp-bind--hidden="!context.isExpanded"
			><!-- minus icon --></span
		...
		<span
			...
			class="prc-show-more__expand-button__label"
		></span>
	</button>
</div>
```
