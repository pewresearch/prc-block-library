...
| API        | Version 3                                                            |
| Textdomain | `timeline`                                                           |
| Example    | Yes (two `timeline-slide` blocks with paragraphs — inserter preview) |
...
| `tickMarkColor`      | `string`  | `""`         | Theme color preset slug (or legacy hex / `rgb()` / `var()`). Set from the Color sidebar. |
...
| ---------------- | ------------------------------------------------------------------------- |
| `prc-block/tabs` | Converts each slide into a tab, using the slide `label` as the tab label. |
...
-   **Tick marks** -- Theme palette color for tick strokes and labels (via `ColorGradientSettingsDropdown`). Cleared state inherits surrounding text color. New selections store a preset slug; older content may still use a raw hex or `rgb()` value, which PHP outputs as-is.
...
-   **Tick Mark Width (px)** -- Range (1 - 12px), tick line thickness.
-   **Tick Label Angle** -- Angle picker to rotate labels for readability.

### Specific Tick Selection

An advanced panel listing every slide by its `label`. Each tick has a toggle to individually include/exclude it. When any specific ticks are selected, this overrides interval and "show all" settings. Includes "Clear Selection" and "Select All" buttons.

## Usage Instructions

1. Insert a **Timeline** block or choose one of the year-based variations from the inserter.
2. Each child `timeline-slide` block represents one step on the timeline. The slide's `metadata.name` is used as the tick label (legacy content may still use the `label` attribute until migrated in the editor).
...
6. Sets CSS custom properties `--tick-height`, `--tick-label-angle`, `--tick-width`, and optionally `--tick-color` inline. For `tickMarkColor`, preset slugs become `var(--wp--preset--color--{slug})`; values that look like hex, `rgb()`/`rgba()`, or `var(...)` are passed through unchanged for backward compatibility.
...
```html
<div
	class="wp-block-prc-block-timeline"
