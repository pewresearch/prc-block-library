...
> **Deprecated.** Use a core **Group** block with the **Grid** layout instead. As of Gutenberg 23.3 the core grid layout supports responsive per-viewport column counts, spacing, and column/row spans natively. PRC adds the two pieces core lacks directly to `core/group` grid: **responsive column order** (Dimensions panel, per Desktop/Tablet/Mobile) and **grid-aware dividers** (color/style/inset). This block is hidden from the inserter but still renders existing content; select a Responsive Grid block and use the **Transform → Group** option to migrate. See [core-group/README.md](../src/core-group/README.md).
...
```html
<div
	class="wp-block-prc-block-grid-controller has-divider has-ui-gray-light-divider-color is-vertically-aligned-top"
...
>
	<div
		class="wp-block-prc-block-grid-column is-vertically-aligned-top"
