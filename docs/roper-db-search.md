...
```html
<div class="wp-block-prc-block-roper-db-search">
  <iframe src="https://ropercenter.cornell.edu/pewglobal/"
...
- Registers Roper-specific query vars (`qid`, `cntIDs`, `stdIDs`, `keyword`, `keywordtext`, `startdate`, `enddate`) via the `prc_platform_rewrite_query_vars` filter.
