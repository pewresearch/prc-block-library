...
-   **`pseudoAttributes`** merges the **first inner** `core/button`’s attributes from the block editor store (`useSelect` + `getBlock(clientId).innerBlocks[0]`) with fixed overrides: `text: 'Reset'` and `backgroundColor: 'ui-white'`.
-   The merge is computed with **`useMemo`** keyed on those inner attributes so the preview updates whenever the template button changes, without holding a stale copy in local state. This keeps the preview aligned with the canonical editor store (including real-time collaboration scenarios).
