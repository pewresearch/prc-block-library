...
const initThresholdWatcher = block => {
   ...
   * @returns
   */
  const toggleTitle = function (groupElm) {
    let matched = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (!groupElm) {
      return;
    }
    const titleElm = groupElm.querySelector('.toc-title');
  ...
  const handleIconConversion = groupElm => {
    ...
    if (isMobileToc) {
      ...
      const itemsWithoutIcons = Array.from(groupElm.querySelectorAll('a.item')).filter(item => !item.getAttribute('data-icon-src'));
  ...
  const toggleGroupBlock = function (groupElm) {
    let matched = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (!groupElm) {
      return;
    }
