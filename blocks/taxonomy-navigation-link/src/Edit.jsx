...
const ALLOWED_BLOCKS = ['prc-block/taxonomy-navigation-link'];
...
export default function Edit({
	...
	const { allowedBlocks, label, showSubMenu } = attributes;
	...
	return (
		<Fragment>
			...
			<div {...blockProps}>
				<RichText
					tagName="span"
					className="wp-block-prc-block-library-taxonomy-navigation-link__label"
					value={label}
					onChange={(newLabel) => setAttributes({ label: newLabel })}
					placeholder={__('Add label…', 'prc-block-library')}
					multiline={false}
				/>
				{showSubMenu && <div {...innerBlocksProps} />}
			</div>
		</Fragment>
	);
}
